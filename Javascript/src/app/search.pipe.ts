...
export class SearchPipe implements PipeTransform {
  /*
  transform(value) {
    // on teste comment fonctionne le filtre sur un array en 1 ligne :
       return value.filter((item) => item.firstname.toLowerCase().includes('ar'));
   }
  */
   transform(value, param) {
     if(param == undefined) {return value}
     else {
     return value.filter((item) =>item.firstname.toLowerCase().includes(param.toLowerCase()));
     }
   }
